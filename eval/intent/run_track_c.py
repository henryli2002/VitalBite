"""Track C — gemma-4-e4b-it-4bit via omlx, zero-shot.

Gemma4 is a multimodal model but this runner uses text-only intent routing
(image markers embedded as <attached_image uuid=.../> text).

Run:
    python eval/intent/run_track_c.py
    python eval/intent/run_track_c.py --model gemma-4-e4b-it-bf16
"""
...
def run(dataset_path: Path, base_url: str, model: str) -> None:
    ...
    trace_path = results_dir / "trace_track_c.jsonl"
    ...
    print_report(traces, track="C")
    write_trace(traces, trace_path)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Track C: Gemma4-4B zero-shot via omlx")
    parser.add_argument(
        "--dataset",
