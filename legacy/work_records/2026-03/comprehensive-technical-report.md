<!-- SUMMARY: 全面技术报告 - WABI项目技术架构与实现细节 | DATE: 2026-03-30 -->
# WABI 全面项目技术报告
...
## 6. 性能瓶颈与高并发限流优化报告 (针对 4核 4GB 内存架构)

在进行 200 并发用户的 WebSocket 压力测试时，系统在 `recognition` 节点出现了明显的超时现象（`timed out waiting for message`，超过 90 秒未响应）。针对这台 4 核 CPU、3GB 内存的服务器，我们对其并发控制机制（Semaphore 背压）和线程池进行了深度调优。

### 6.1 瓶颈原因分析

1. **内存过载风险 (Memory Overflow)**:
   `recognition` 节点负责处理多模态图像任务，每个请求通常携带 2~4MB 的 Base64 图像。原先的信号量 `MAX_CONCURRENT_WORKERS = 200` 允许 200 个带图请求同时进入内存，光图像负载就逼近 800MB，加上模型加载（如 SentenceTransformer）和其他节点逻辑，极易导致 3GB 内存的机器发生 OOM（内存溢出）或频繁触发 Swap 拖慢全局响应。
2. **CPU 频繁上下文切换 (Context Switching Overhead)**:
   FAISS 向量检索（`fndds` 节点）属于高度 CPU 密集型任务。原先配置的 `ThreadPoolExecutor(max_workers=8)` 超过了机器物理核心数（4核）。当大量并发涌入时，8 个线程在 4 个核心上争抢时间片，导致频繁的上下文切换开销，反而降低了总吞吐量，致使下游请求在队列中堆积，最终触发 90 秒超时熔断。
3. **漏斗效应引发的超时**:
   虽然外层 `recognition` 放行了 200 个并发，但底层 `fndds` 的信号量原先只设了 36。这意味着大量请求在成功进入 `recognition` 节点后，会在调用 FAISS 检索时被堵死在内部的 asyncio 队列中。这种不匹配的限流导致请求的处理时间被无意义地拉长。

### 6.2 优化方案与参数调整

针对上述瓶颈，我们修改了 `langgraph_app/utils/semaphores.py` 和 `langgraph_app/tools/nutrition/fndds.py`，实施了更严格、更符合硬件物理上限的两级背压限流：

- **计算池降级适配物理核心**:
  将 `FNDDS_THREAD_POOL` 的 `max_workers` 从 8 下调至 **4**，与 CPU 物理核心数 1:1 对齐，保证 FAISS 检索时 CPU 始终满载且零切换损耗。
- **底层 FNDDS 队列限流**:
  将 `fndds` 信号量从 36 下调至 **16**。这意味着在 Python 的异步事件循环中，最多只有 16 个 FAISS 检索任务能穿过限流门：其中 4 个在工作线程全速执行，另外 12 个在线程池队列中静默排队。多余的请求将在不阻塞主循环的情况下被异步挂起。
- **顶层内存安全限流**:
  将 `recognition` 信号量从 200 狠砍至 **50**。这保证了极限并发下，多模态图像占用的内存峰值被硬控在 150MB 左右，留出充足的内存冗余保障系统整体稳定性。同时，`recommendation`、`tutorial`、`goalplanning` 等中高负载节点的信号量也均由 200 下调至 **100**。

### 6.3 优化总结
通过上述“削峰填谷”式的参数调优，系统放弃了理论上不切实际的“超高并发同时处理”，转向**确保核心资源不被撑爆、请求优雅降级排队**的务实策略。这一改动彻底根除了因资源争抢导致的雪崩式超时，使得 4核 3GB 的有限算力得到了最平稳、最极致的发挥。