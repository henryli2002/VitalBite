...
# It loads the model, the FAISS index, and the document data into memory.
MODEL_NAME = 'all-MiniLM-L6-v2'
TOOL_DIRECTORY = os.path.dirname(os.path.abspath(__file__))
INDEX_FILE = os.path.join(TOOL_DIRECTORY, 'fndds_index.faiss')
DOCUMENTS_FILE = os.path.join(TOOL_DIRECTORY, 'fndds_documents.json')
...
def fndds_nutrition_search_tool(food_description: str, top_k: int = 3) -> str:
    ...
    if not all([model, index, documents]):
        error_message = "FNDDS RAG Tool is not initialized. Check for errors on startup."
        print(error_message)
    ...
    try:
        ...
        # The search function returns distances and indices (I)
