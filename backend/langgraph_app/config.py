...
class Config:
    ...
    BEDROCK_CLAUDE_TEMPERATURE: float = float(
        os.getenv("BEDROCK_CLAUDE_TEMPERATURE", "0.2")
    )
    ...
    def get_history_count(cls, agent_name: str) -> int:
        ...
        return (
            agent_specific if agent_specific is not None else cls.HISTORY_MESSAGE_COUNT
        )
